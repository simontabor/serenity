...
var Generator = module.exports = function(files, config) {
  var self = this;
  ...
  self.deleteSite(function() {
    ...
    files.forEach(function(file) {
      ...
      fs.mkdirs('./_site/' + dir + '/',function(err) {
...
Generator.prototype.compile = function(file, asset, cb) {
  asset.compile(function(err, ass) {
    ...
    if (file.extension === 'js') {
      info = UglifyJS.minify(info, {fromString: true}).code;
      return cb(info);
    }
...
Generator.prototype.save = function(file, data, cb) {
  ...
  var filepath = file.relative.replace('less/','css/').replace(/\.less$/,'.css').replace(/\.ejs$/, '.html');
