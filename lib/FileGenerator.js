...
FileGenerator.prototype.compile = function(asset, cb) {
  ...
  asset.compile(function(err, ass) {
    ...
    if (file.extension === 'js') {
      if (self.generator.config.minify.js) {
...
FileGenerator.prototype.renderEJS = function(cb) {
  ...
  ejs.renderSerenity(file.data, {
    ...
    if (self.generator.config.markdown) {
      if (self.generator.config.markdown.highlight) {
        self.generator.config.markdown.highlight = function(code, lang) {
          if (lang === 'no-hl') return code;
      ...
      html = html.replace(
        /<(?:markdown|md)>((?:.*?\n*?)*?)<\/(?:markdown|md)>/g,
        function(match, body){
          return marked(body, self.generator.config.markdown);
        }
      );
    }
    ...
    if (self.generator.config.minify.css) {
      html = html.replace(
        ...
        function(match, open, body, close){
          return open + cleanCss().minify(body) + close;
        }
      );
    }
    ...
    if (self.generator.config.minify.html) {
